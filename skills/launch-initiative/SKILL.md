...
5. **Attach the opener**: `whatsapp_numbers_list` → pick/create the template (see `whatsapp-templates`) → `initiatives_templates_set`. Meta approval is usually minutes for text UTILITY templates, but close the copy with the user before creating one: a submitted template can never be edited or deleted.
6. **Set the extraction schema before launching**, if the mission needs structured fields back (`extraction_schema_set`). A conversation is extracted against whichever schema was current when it ran, so declare the fields now rather than after the first results come in. Adding one later is recoverable but not free: `extraction_reextract` re-runs extraction over conversations that already happened, and it fans out one model call each, so it costs real money. Six field types (`bool`, `int`, `enum`, `enum[]`, `string`, `string[]`); `topic`, `problem`, and `competitor` are reserved slugs that get rejected. Each field's `description` is the instruction the model reads to fill it, not a label, so write it like a briefing. Extraction only runs on conversations with at least 3 messages and 1 inbound reply: a cold audience that stays silent yields no fields for whoever never answers.

   Unlike the guiding questions, **the extraction schema is not write-once**: `extraction_schema_set` works on an initiative that is already running, and does not touch the questions. So when what you need is for an answer to come back **typed, filterable and exportable**, you can add the field without recreating anything, even if the agent is already asking about it in prose. It replaces the schema wholesale rather than merging, so read with `extraction_schema_get` first and send the complete set back. Give every enum a "did not say" member: without one the model is forced to pick among options that do not apply, and the noise lands in the exact field you meant to segment on.
