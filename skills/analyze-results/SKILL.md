...
- If the user wants an answer that is buried in the prose of the transcripts, check whether it should be a typed field instead. `extraction_schema_set` works on a running initiative, so a field can be added without recreating anything. Conversations that already ran keep the schema they ran under, but `extraction_reextract` goes back over them under the new one: call it with `dryRun: true` first, which reports how many conversations are eligible and how many carry enough transcript to yield anything. It fans out one model call per conversation, so it costs real money and takes the same admin gate as authoring the schema. What a conversation extracts stays scoped to its own initiative, so it will not show up in another one's results.
